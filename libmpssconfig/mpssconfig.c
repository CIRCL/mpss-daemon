	   ...
	   struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((mic->config.boot.osimage = malloc(strlen(image) + 1)) != NULL) {
	    ...
	    struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((mic->config.boot.efiimage = malloc(strlen(image) + 1)) != NULL) {
	   ...
	   struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	case NETWORK_TYPE_STATPAIR:
		...
		if ((bits > 24) || (bits < 9)) {
			...
			if ((mic->config.net.prefix = malloc(3)) != NULL)
				strcpy(mic->config.net.prefix, "24");
		} else {
			if ((mic->config.net.prefix = malloc(strlen(netbits) + 1)) != NULL)
				strcpy(mic->config.net.prefix, netbits);
		}

		if (mtu != NULL) {
			if ((mic->config.net.mtu = malloc(strlen(mtu) + 1)) != NULL)
		...
		if (modcard) {
			if ((mic->config.net.modcard = malloc(strlen(modcard)+1)) != NULL)
	...
	case NETWORK_TYPE_STATBRIDGE:
		...
		if ((mic->config.net.micIP = malloc(strlen(micip0) + 1)) != NULL)
		...
		if ((mic->config.net.bridge = malloc(strlen(bridge) + 1)) != NULL)
	...
	case NETWORK_TYPE_BRIDGE:
		...
		if ((mic->config.net.micIP = malloc(strlen("dhcp") + 1)) != NULL)
       ...
       struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if (!strcasecmp(type, "Serial")) {
		...
		if ((mic->config.net.micMac = malloc(strlen(micmac) + 1)) != NULL) {
			strcpy(mic->config.net.micMac, micmac);
		}

		if ((mic->config.net.hostMac = malloc(strlen(hostmac) + 1)) != NULL) {
	   ...
	   struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((dir->next = malloc(sizeof(struct moverdir))) == NULL) {
	...
	if ((dir->dir = malloc(strlen(source) + 1)) == NULL) {
		add_perr(perrs, PERROR, "[Parse] Failed to alloc memory for overlay");
		return 0;
	}
	...
	if (otype != OVER_TYPE_RPM) {
		if ((dir->target = malloc(strlen(target) + 1)) == NULL) {
			free(dir->dir);
			return 0;
		}
	   ...
	   struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if (!strcasecmp(roottype, "ramfs")) {
		...
		if (rootpath == NULL) {
			...
			if ((mic->config.rootdev.target = malloc(strlen(rootpath) + 1)) == NULL)
				return 0;
		...
		if (rootpath == NULL) {
			...
			if ((mic->config.rootdev.nfsusr = malloc(strlen(usrpath) + 1)) == NULL)
				return 0;
			strcpy(mic->config.rootdev.nfsusr, usrpath);
		}
	} else if (!strcasecmp(roottype, "pfs")) {
		mic->config.rootdev.type = ROOT_TYPE_PFS;
		if (rootpath == NULL) {
			add_perr(perrs, PERROR, "%s: [Parse] RootDevice specifies Persistent FS but no PFS image defined", mic->name);
		} else {
			if ((mic->config.rootdev.target = malloc(strlen(rootpath) + 1))  == NULL)
				return 0;
			strcpy(mic->config.rootdev.target, rootpath);
	    ...
	    struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((mic->config.net.hostname = malloc(strlen(name) + 1)) == NULL) {
		return 0;
	}
	 ...
	 struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((mic->config.boot.extraCmdline = malloc(strlen(cmdline) + 1)) == NULL) {
		return 0;
	}
	   ...
	   struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((mic->config.boot.console = malloc(strlen(console) + 1)) == NULL) {
		return 0;
	}
      ...
      struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((mic->config.boot.pm = malloc(strlen(pm) + 1)) == NULL) {
		return 0;
	}
	       ...
	       struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((mic->config.misc.shutdowntimeout = malloc(strlen(timeout) + 1)) == NULL) {
		return 0;
	}
	     ...
	     struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((mic->config.misc.crashdumpDir = malloc(strlen(dirname) + 1)) == NULL)
		return 0;
	strcpy(mic->config.misc.crashdumpDir, dirname);

	if ((mic->config.misc.crashdumplimitgb = malloc(strlen(limit) + 1)) == NULL)
		return 0;
	   ...
	   struct mpss_elist *perrs, int level, char *cfile, int lineno)
{
	...
	if ((serv->next = malloc(sizeof(struct mservice))) == NULL)
		return 0;
	serv = serv->next;

	if ((serv->name = malloc(strlen(name) + 1)) == NULL)
		return 0;
...
add_perr(struct mpss_elist *perrs, unsigned char level, char *format, ...)
{
	...
	if ((elem = malloc(sizeof(struct melist))) == NULL)
		return;
	...
	if ((elem->string = malloc(strlen(buffer) + 1)) != NULL)
		  ...
		  char *filename, struct mpss_elist *perrs, int level)
{
	...
	if ((file_image = malloc(sbuf.st_size + 1)) == NULL) {
		close(fd);
...
_get_miclist_present(struct mpss_env *menv, int *cnt)
{
	...
	*cnt = 0;
	...
	while ((file = readdir(dp)) != NULL) {
		...
		if ((mic->next = malloc(sizeof(struct mic_info))) != NULL) {
			mic = mic->next;
...
_add_miclist_not_present(struct mpss_env *menv, struct mic_info *miclist, int *cnt)
{
	...
	while ((file = readdir(dp)) != NULL) {
		...
		while (mic->next) {
			if (mic->next->id > id) {
				if ((tmp = malloc(sizeof(struct mic_info))) != NULL) {
...
bool is_upstream_driver(void)
/*
* TBD: Change the way of how mpss version is checked
*/
{
	static bool ischecked = false;
	static bool isupstream;

	if (!ischecked) {
		int fd;
		char filename[PATH_MAX];

		/* Presence of /sys/class/mic/scif indicates a production driver */
		snprintf(filename, PATH_MAX - 1, "%s/%s", MICSYSFSDIR, "scif");
		if ((fd = open(filename, O_RDONLY)) >= 0) {
			close(fd);
			isupstream = false;
		} else {
...
bool is_mic_knc(struct mic_info *mic)
{
	static bool ischecked = false;
	static bool ismicknc;

	if (!ischecked) {
		char *family;
		if ((family  = mpss_readsysfs(mic->name, "family")) == NULL) {
			ischecked = true;
			return false;
		}
...
bool is_mic_knl(struct mic_info *mic)
{
	static bool ischecked = false;
	static bool ismicknl;
	...
	if (!ischecked) {
		...
		if (!strcmp(family, MIC_FAMILY_KNL))
			ismicknl = true;
		else
			ismicknl = false;
