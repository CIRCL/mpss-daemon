...
int
mpssut_realloc_fill(char **addr, char *new)
{
	if (*addr != NULL)
		free(*addr);

	*addr = mpssut_alloc_fill(new);
	return 0;
}
...
mpssut_configname(struct mpss_env *menv, char *name)
{
	...
	if ((buf = malloc(strlen(buffer) + 1)) != NULL) {
		strcpy(buf, buffer);
	}
...
mpssut_tilde(struct mpss_env *menv, char *name)
{
	...
	if (*name == '~') {
		...
		if ((newname = malloc(strlen(menv->home) + strlen(name) + 2)) == NULL)
			return NULL;

		sprintf(newname, "%s%s", menv->home, &name[1]);
	} else {
		if ((newname = malloc(strlen(name) + 1)) == NULL)
			return NULL;
