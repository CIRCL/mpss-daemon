...
int insert_list(int id, char *path, struct filetree *new, struct filetree *elem, char *source, struct mpss_elist *perrs);
...
handle_file(int id, struct filetree *top, char *source, char *dest, struct mpss_elist *perrs)
{
	...
	if ((new = malloc(sizeof(struct filetree))) == NULL)
		return 0;

	new->type = MPSS_FILE;
	if ((new->source = malloc(strlen(source) + 1)) == NULL) {
		free(new);
		return 0;
	}
	...
	return insert_list(id, &dest[1], new, top, NULL, perrs);
}
...
_handle_rpm(int id, struct filetree *top, char *source, struct mpss_elist *perrs)
{
	...
	return insert_list(id, dest, new, top, NULL, perrs);
}
...
insert_name(int id, char *name, struct mpss_elist *perrs)
{
	...
	if ((new = malloc(strlen(name) + 1)) == NULL) {
...
alloc_and_fill_missing_entry(int id, char *name, struct mpss_elist *perrs)
{
	...
	memset(new, 0, sizeof(struct filetree));

	if ((new->name = insert_name(id, name, perrs)) == NULL) {
		free(new);
		return NULL;
	}
...
int
insert_subdir(int id, struct filetree *cur, struct filetree *new, char *name, char *rest,
	      char *source, struct mpss_elist *perrs)
{
	...
	return insert_list(id, rest, new, cur->subdir, source, perrs);
}

int
insert_top(int id, struct filetree *cur, struct filetree *new, char *name, char *rest,
	      char *source, struct mpss_elist *perrs)
{
	struct filetree *tmp;
	...
	cur->subdir->prev = tmp;
	cur->subdir = tmp;
	return insert_list(id, rest, new, tmp, source, perrs);
}

int
insert_before(int id, struct filetree *cur, struct filetree *new, char *name, char *rest,
	      char *source, struct mpss_elist *perrs)
{
	struct filetree *tmp;
...
int
overwrite_entry(int id, struct filetree *cur, struct filetree *new, char *name, char *rest,
	        char *source, struct mpss_elist *perrs)
{
	if (rest != NULL) {
		return insert_list(id, rest, new, cur, source, perrs);
	}
	...
	if (cur->type != new->type) {
		add_perr(perrs, PINFO, "mic%d: overlay %s abort mismatched types changing from %s to %s",
			      id, name, get_type_string(cur->type), get_type_string(new->type));
		return 1;
	}

	// Update the old entry to match and free the new.
	if (cur->source != NULL)
		free(cur->source);
...
int
insert_list(int id, char *path, struct filetree *new, struct filetree *elem, char *source, struct mpss_elist *perrs)
{
	struct filetree *tmp;
	char *cur = path;
	char *rest = cur;
	...
	if (elem->subdir == NULL)	// There are currently no files in this directory.
		return insert_subdir(id, elem, new, cur, rest, source, perrs);

	if (strcmp(cur, elem->subdir->name) < 0)
		return insert_top(id, elem, new, cur, rest, source, perrs);
	...
	while (elem != NULL) {
		...
		if (miss < 0) {	// Insert before the cur element
			return insert_before(id, elem, new, cur, rest, source, perrs);
		} else if (miss == 0) {	// Exact match
			return overwrite_entry(id, elem, new, cur, rest, source, perrs);
	...
	// Add to tail
	if (rest == NULL) {
		if ((new->name = insert_name(id, path, perrs)) == NULL) {
			free(new);
			return 1;
		}
	...
	elem = tmp->next;
	elem->next = NULL;
	return insert_list(id, rest, new, elem, source, perrs);
}
...
add_dir(int id, char *line, struct filetree *top, char *sourcedir, struct mpss_elist *perrs)
{
	...
	if (sscanf(line, "%" str(PATH_MAX) "s %o %d %d", path, &new->mode, &new->uid, &new->gid) != 4) {
	...
	insert_list(id, &path[1], new, top, NULL, perrs);
}
...
add_file(int id, char *line, struct filetree *top, char *sourcedir, struct mpss_elist *perrs)
{
	...
	if (sscanf(line, "%" str(PATH_MAX) "s %" str(PATH_MAX) "s %o %d %d",
		   path, loc, &new->mode, &new->uid, &new->gid) != 5) {
		add_perr(perrs, PWARN, "mic%d Bad attribute line 'dir %s'", id, line);
	}
	...
	if ((new->source = malloc(strlen(sourcedir) + strlen(loc) + 3)) == NULL) {
		free(new);
		return;
	}
	sprintf(new->source, "%s/%s", sourcedir, loc);
...
add_link(int id, char *line, struct filetree *top, char *sourcedir, struct mpss_elist *perrs)
{
	...
	if (sscanf(line, "%" str(PATH_MAX) "s %" str(PATH_MAX) "s %o %d %d",
		   path, target, &new->mode, &new->uid, &new->gid) != 5) {
		add_perr(perrs, PWARN, "mic%d Bad attribute line 'slink %s'", id, line);
	}

	new->type = MPSS_LINK;

	if ((new->link = malloc(strlen(target) + 1)) == NULL) {
		free(new);
		return;
	}

	strcpy(new->link, target);
	new->subdir = NULL;
	new->next = NULL;
...
add_simple_link(int id, char *source, char *target, char *lead, struct filetree *top, struct mpss_elist *perrs)
{
	...
	if ((new->link = malloc(strlen(rbuf) + 1)) == NULL) {
		free(new->source);
		free(new);
		return;
	}
	memset(new->link, 0, strlen(rbuf) + 1);
	strcpy(new->link, rbuf);
...
add_dnode(int id, char *line, struct filetree *top, char *sourcedir, struct mpss_elist *perrs)
{
	...
	if (sscanf(line, "%" str(PATH_MAX) "s %o %d %d %c %u %u", path, &new->mode, &new->uid,
		   &new->gid, &new->inode_type, &new->major, &new->minor) != 7) {
		add_perr(perrs, PWARN, "mic%d Bad attribute line 'dir %s'", id, line);
	}
	 ...
	 struct mpss_elist *perrs)
{
	...
	if ((filebuf = malloc(sbuf.st_size)) == NULL) {
...
follow_dir(struct mpss_env *menv, struct filetree *elem, char *leader, FILE *fp, unsigned int *inode, unsigned int *offset, struct mpss_elist *perrs)
{
	...
	while (elem != NULL) {
		...
		case MPSS_DIR:
			...
			if ((newleader = malloc(strlen(leader) + strlen(elem->name) + 2)) == NULL) {
				return;
			}
