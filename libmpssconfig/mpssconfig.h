...
**/
void boot_media(int node, const char *media_path);
...
#define DEFAULT_PFS_KNL	"persistent-FS-knl-lb.ext3.gz"
#endif
...
/**
 * is_upstream_driver - Check if the underlying driver is the recently
 * up-streamed driver
 *
 * is_upstream_driver looks for the interface /sys/class/mic/scif. Since
 * this interface is specific to the production driver, absense of it indicates
 * an upstream driver.
 *
 * \return
  Return true if the driver is the upstream driver, false otherwise
 */
bool is_upstream_driver(void);

/**
 * is_mic_knc - Check to see if the MIC device is a KNC
 *     \param mic	mic_info data structre with valid config data
 *
 * is_mic_knc looks for 'x100' inside /sys/class/mic/<name>/family
 *
 * \return
  Return true if the MIC device is a KNC, false otherwise
 */
bool is_mic_knc(struct mic_info *mic);

/**
 * is_mic_knl - Check to see if the MIC device is a KNL
 *     \param mic	mic_info data structre with valid config data
 *
 * is_mic_knl looks for 'x200' inside /sys/class/mic/<name>/family
 *
 * \return
  Return true if the MIC device is a KNL, false otherwise
 */
