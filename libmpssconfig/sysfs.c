...
mpss_readsysfs(char *name, char *entry)
{
	...
	*/
	...
	if ((string = malloc(strlen(valstart) + 1)) != NULL)
...
get_console(struct mic_info *mic)
{
	...
	if ((cmd_string = malloc(strlen(mic->config.boot.console) + strlen("console=") + 1)) == NULL)
		return NULL;
...
get_cgroup(struct mic_info *mic)
{
	...
	if ((cmd_string = malloc(strlen("cgroup_disable=memory") + 1)) == NULL)
		return NULL;
