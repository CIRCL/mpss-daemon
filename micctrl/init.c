...
char *kncextracmdupstreamdriver =
	"ExtraCommandLine \"highres=off\"\n\n";
char *knlextracmd =
	"ExtraCommandLine \"nohz=off clocksource=tsc\"\n\n";
...
create_default_config_files(struct mic_info *mic)
{
	...
	if (!mpssut_filename(&mpssenv, NULL, filename, PATH_MAX, "%s/default.conf", mpssenv.confdir)) {
		...
		if (!is_mic_knc(mic)) {
			fprintf(fp, knlextracmd);
		} else {
			if (is_upstream_driver())
				fprintf(fp, kncextracmdupstreamdriver);
			else
...
set_base(struct mic_info *mic)
{
	...
	if (mic->config.filesrc.base.image == NULL) {
		if (is_mic_knl(mic)) {
			snprintf(imagename, PATH_MAX, "%s/%s", mpssenv.srcdir, DEFAULT_INITRD_KNL);
		} else {
...
set_osimage(struct mic_info *mic)
{
	...
	char efiname[PATH_MAX];

	if (is_mic_knl(mic)) {
		snprintf(bzname, PATH_MAX, "%s/%s", mpssenv.srcdir, DEFAULT_BZIMAGE_KNL);
		snprintf(mapname, PATH_MAX, "%s/%s", mpssenv.srcdir, DEFAULT_SYSMAP_KNL);
		snprintf(efiname, PATH_MAX, "%s/%s", mpssenv.srcdir, DEFAULT_EFIIMAGE_KNL);
	...
	if (is_mic_knl(mic)) {
		if (mic->config.boot.efiimage == NULL) {
			char *efi_descriptor = "# MIC EFI image";
			mic->config.boot.efiimage = mpssut_alloc_fill(efiname);
			mpss_update_config(&mpssenv, mic->config.name, NULL, efi_descriptor,
					   "%s %s \n\n", "EFIimage", mic->config.boot.efiimage);
		}

		display(PINFO, "%s: Linux OS image %s\n%s      EFI image %s\n      System Map %s\n",
			mic->name, mic->config.boot.osimage, display_indent(),
			mic->config.boot.efiimage, mic->config.boot.systemmap);
	} else {
		display(PINFO, "%s: Linux OS image %s\n%s      System Map %s\n", mic->name,
