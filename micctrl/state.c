...
int gen_tmpfs_image(struct mic_info *mic);

/*
 * Boot the list of MIC cards.  If a timeout value is specified then wait for the cards to
	...
	for (mic = miclist; mic != NULL; mic = mic->next) {
		...
		switch(mic->config.family) {
			case MIC_FAMILY_KNL_VALUE:
				if (!is_mic_knl(mic))
					display(PWARN, "%s: Family mismatch. Configuration file parameter Family is %s but MIC family is different\n"
			...
			default:
				display(PWARN, "%s: Family parameter is %s\n\t Please execute micctrl --cleanconfig and --initdefaults again\n", mic->name, family_to_str(mic->config.family));
		}
		...
		if (is_upstream_driver()) {
			...
			if (mpssut_mksubtree(&mpssenv, "", os_image_path, 0, 0, 0755)) {
				display(PERROR, "%s Boot aborted - Failed to create boot directory. %s\n",
			...
			if (symlink(mic->config.boot.osimage, kernel) < 0) {
				display(PERROR, "%s Boot aborted - Failed to create symlink for kernel image", mic->name);
				fail++;
				continue;
			}

			if (symlink(initrd, ramdisk) < 0) {
				display(PERROR, "%s Boot aborted - Failed to create symlink for filesystem", mic->name);
				fail++;
				continue;
			}

			if(is_mic_knl(mic) && symlink(mic->config.boot.efiimage, efiimage) < 0) {
				display(PERROR, "%s Boot aborted - Failed to create symlink for EFI image", mic->name);
				fail++;
				continue;
			}
			...
			if (mpss_setsysfs(mic->name, "bootmode", "linux")) {
				...
				display(PERROR, "%s failed to boot. Bootmode = %s\n", mic->name, sysfs_rd);
				free(sysfs_rd);
				fail++;
				continue;
			}

			//TBD_MIC - fix for KNC - many types of root
			/* request_firmware doesnt like absolute paths. */
			...
			if (mpss_setsysfs(mic->name, "firmware", kernel)) {
				...
				display(PERROR, "%s failed to boot. Firmware = %s\n", mic->name, sysfs_rd);
				free(sysfs_rd);
				fail++;
				continue;
			}
			...
			if (mpss_setsysfs(mic->name, "ramdisk", ramdisk)) {
				...
				display(PERROR, "%s failed to boot. Ramdisk = %s\n", mic->name, sysfs_rd);
				free(sysfs_rd);
				fail++;
				continue;
			}
			...
			if (is_mic_knl(mic) && mpss_setsysfs(mic->name, "efiimage", efiimage)) {
				...
				display(PERROR, "%s failed to boot. efiimage = %s\n", mic->name, sysfs_rd);
				free(sysfs_rd);
				fail++;
				continue;
			}
...
reset(struct mic_info *miclist, int timeout, int force, int ignore)
{
	...
	for (mic = miclist; mic != NULL; mic = mic->next) {
		if (!is_upstream_driver()) {
			...
			if (force) {
				display(PINFO,
				"%s: forced reset only supported through the soft_reset debugfs interface\n",
				mic->name);
			}
	...
	for (mic = miclist; mic != NULL; mic = mic->next) {
		...
		if (state == NULL) {
			...
			if (is_upstream_driver()) {
				...
				image = mpss_readsysfs(mic->name, "firmware");
			} else {
...
{
	bool isupstreamdriver = is_upstream_driver();
		...
		if (states[mic->id]) {
			if (isupstreamdriver) {
				if (strcmp(states[mic->id], "online") && strcmp(states[mic->id], "ready")
					&& strcmp(states[mic->id], "suspended"))
					bad_state++;
			} else {
				if (strcmp(states[mic->id], "online") && strcmp(states[mic->id], "ready"))
					bad_state++;
