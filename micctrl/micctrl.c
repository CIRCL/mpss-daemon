...
static struct option main_longopts[] = {
	...
	{"efiimage", optional_argument, NULL, 0x1027},
	{0}
};
...
\n\
        --osimage    Set the OSimage parameter to the locations to find the\n\
                     ...
                     RPM.\n\
\n\
        --efiimage   Set the EFIimage parameter to the location to find the\n\
                     EFI FAT32 image to boot the card instead of the default\n\
                     version in the mpss-boot-file RPM.\n\
...
main(int argc, char *argv[], char *envp[])
{
	...
	while ((c = getopt_long(argc, argv, "hbrSRswc:d:", main_longopts, &longidx)) != -1) {
		...
		case 0x1027:
			parse_efiimage_args(optarg, argc, argv);
			break;
