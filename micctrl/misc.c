...
char *efiimage_help =
"\
micctrl [global options] --efiimage=<efiimage> [sub options] [Xeon Phi cards]\n\
\n\
The --efiimage option sets the EFI FAT32 disk image to boot from.\n\
The initdefaults would have set the default image to point to the image installed\n\
with the mpss-boot-files RPM.\n\
\n\
If no efiimage is specified the current settings will be displayed.\n\
\n"
COMMON_GLOBAL
COMMON_HELP
"\n";

static struct option efiimage_longopts[] = {
	{"help", no_argument, NULL, 'h'},
	{0}
};

void show_efiimages(struct mic_info *miclist);
void set_efiimage(struct mic_info *miclist, char *image);

void
parse_efiimage_args(char *efiimage, int argc, char *argv[])
{
	struct mic_info *miclist;
	int cnt;
	int longidx;
	...
	while ((c = getopt_long(argc, argv, "hv:", efiimage_longopts, &longidx)) != -1) {
		switch(c) {
		case 'h':
			micctrl_help(efiimage_help);
			exit(0);
		...
		default:
			ARGERROR_EXIT(efiimage_help);
		}
	}

	miclist = create_miclist(&mpssenv, argc, argv, optind, &cnt);
	if (miclist != NULL && !is_mic_knl(miclist)) {
		display(PWARN, "%s: This option should be executed only on KNL environment\n", miclist -> name);
	}

	if (efiimage == NULL) {
		show_efiimages(miclist);
	} else {
		check_rootid();
		set_efiimage(miclist, efiimage);
	}
}
...
void config_service(struct mic_info *miclist, char *service, int start, int stop, char *state);
...
void
show_efiimages(struct mic_info *miclist)
{
	struct mic_info *mic;
	...
	for (mic = miclist; mic != NULL; mic = mic->next) {
		...
		printf("%s:\tefiimage: %s\n", mic->name, mic->config.boot.efiimage);
	}
...
void
set_efiimage(struct mic_info *miclist, char *image)
{
	struct mic_info *mic;
	char filename[PATH_MAX];
	struct stat sbuf;
	...
	for (mic = miclist; mic != NULL; mic = mic->next) {
		...
		if ((err = mpss_update_config(&mpssenv, mic->config.name, "EFIimage", NULL,
									"EFIimage %s\n", image)))
			display(PERROR, "Failed to open config file '%s': %d\n", mic->config.name, err);
		else
			display(PFS, "%s: Update EFIimage in %s\n", mic->name, mic->config.name);
	}
...
void
mod_service(struct mic_info *mic, struct mservice *serv, int start, int stop, char *state)
{
...
void
config_service(struct mic_info *miclist, char *service, int start, int stop, char *state)
{
	struct mic_info *mic;
	struct mservice *serv;
	char filename[PATH_MAX];
