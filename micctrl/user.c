	...
	char *app, char *shell, char *sshkeydir, int createhome, int unique)
{
	...
	if (home == NULL) {
		if ((home = malloc(strlen("/home/ ") + strlen(name))) == NULL) {
			memory_alloc_error();
		}
	...
	if (comment == NULL) {
		if ((comment = malloc(strlen("User Account  ") + strlen(name))) == NULL) {
			memory_alloc_error();
		}
...
fillin_userinfo:
	...
	if (sshkeydir == NULL) {
		if ((sshkeydir = malloc(strlen("/home//.ssh ") + strlen(name))) == NULL) {
			memory_alloc_error();
		}
...
create_root_user(void)
{
	...
	if ((ulhead = malloc(sizeof(struct micuser))) == NULL) {
		return NULL;
	}

	memset((void *)ulhead, 0, sizeof(struct micuser));

	if ((ulhead->user = malloc(strlen("root") + 1)) == NULL) {
		free(ulhead);
		return NULL;
	}
	...
	while (ulist != NULL) {
		...
		while(fgets(line, 1024, shfp)) {
			...
			if (!strcmp(ulist->user, user)) {
				...
				if ((ulist->shadow = malloc(strlen(buffer) + 1)) == NULL)
					goto nextshadow;
...
fill_user(char *user, uid_t uid, gid_t gid, char *name, char *home, char *app)
{
	...
	if ((mu = malloc(sizeof(struct micuser))) == NULL)
		return NULL;

	memset((void *)mu, 0, sizeof(struct micuser));

	if ((mu->user = malloc(strlen(user) + 1)) != NULL)
		strcpy(mu->user, user);
	...
	if (name != NULL)
		if ((mu->name = malloc(strlen(name) + 1)) != NULL)
			strcpy(mu->name, name);

	if (home != NULL)
		if ((mu->home = malloc(strlen(home) + 1)) != NULL)
			strcpy(mu->home, home);

	if (app != NULL)
		if ((mu->app = malloc(strlen(app) + 1)) != NULL)
			strcpy(mu->app, app);
...
fill_group(char *group, gid_t gid, char *users)
{
	...
	if ((mg = malloc(sizeof(struct micgrp))) == NULL)
		return NULL;

	memset((void *)mg, 0, sizeof(struct micgrp));

	if ((mg->group = malloc(strlen(group) + 1)) != NULL)
		strcpy(mg->group, group);

	mg->gid = gid;

	if (users != NULL)
		if ((mg->users = malloc(strlen(users) + 1)) != NULL)
	 ...
	 char *pwname, char *shname)
{
	...
	if (passwd_str != NULL) sprintf(passwd_str, "%s:x:%u:%u:%s:%s:%s\n", user, uid, gid, name, home, app);

	if (shadow != NULL) {
		fprintf(shfp, "%s", shadow);
		if (shadow_str) sprintf(shadow_str, "%s", shadow);
	} else {
		fprintf(shfp, "%s:*:14914::::::\n", user);
		if (shadow_str) sprintf(shadow_str, "%s:*:14914::::::\n", user);
	}
...
add_group(char *micname, FILE *grfp, char *group, gid_t gid, char *group_str, char *grname)
{
	...
	if (group_str != NULL) sprintf(group_str, "%s:x:%u:\n", group, gid);
}
