...
parse_addbridge_args(char *name, int argc, char *argv[])
{
	...
	if (!is_mic_knc(miclist) && mtu != NULL) {
		ARGERROR_EXIT(addbr_help);
	}
...
void
get_mac_strings(struct mic_info *mic, char *micmac, char *hostmac)
{
	char *serial;
	...
	if (mic->config.net.micMac == NULL)
		...
		strcpy(micmac, mic->config.net.micMac);

	if (mic->config.net.hostMac == NULL)
		strcpy(hostmac, "Not Configured");
	else
		strcpy(hostmac, mic->config.net.hostMac);
}
...
show_net_config(struct mic_info *miclist)
{
	...
	for (mic = miclist; mic != NULL; mic = mic->next) {
		...
		get_mac_strings(mic, micmac, hostmac);
...
get_ips(char *list, char **ipbuf)
{
	...
	if ((buffer = malloc(strlen(list) + 1)) == NULL) {
		*ipbuf = NULL;
		return -1;
	}
...
populate_bridgeip(char *ip, int miccnt)
{
	...
	for (i = 1; i < miccnt; i++)
		if ((ips[i][0] = malloc(strlen(ips[0][0]) + 3)) != NULL)
...
write_line:
	...
	if (entry_found == FALSE) {
		...
		display(PINFO, "%s: Update /etc/hosts with %s %s\n", micid, ip, hostname);
	}
...
populate_stpairip(char *ip, int miccnt)
{
	...
	for (i = 0; i < miccnt; i++) {
		if ((ips[i][0] = malloc(16)) != NULL)
			sprintf(ips[i][0], "%d.%d.%d.1", ip1[0], ip1[1], i + 1);
		if ((ips[i][1] = malloc(16)) != NULL)
...
fill_singles(int cnt)
{
	...
	if ((cnt == 1) && mpssnet_decode_ipaddr(ips[0][0], ip1, 4)) {
		...
		if ((ips[0][0] = malloc(16)) != NULL)
			sprintf(ips[0][0], "%d.%d.1.1", ip1[0], ip1[1]);
		if ((ips[0][1] = malloc(16)) != NULL)
	   ...
	   int mhost, char *mcard, struct mic_info *miclist, int miccnt)
{
	...
	if (update) {
		...
		if (is_mic_knc(mic)) {
			...
			mpss_update_config(&mpssenv, mic->config.name, "Network", NULL,
				"Network class=StaticPair micip=%s hostip=%s modhost=%s  modcard=%s netbits=%s\n",
				mic->config.net.micIP, mic->config.net.hostIP,
				mhost? "yes" : "no", mcard? mcard : "yes", netbits);
		}
...
mod_bridge(char *name, char *ip, char *netbits, char *mtu, struct mic_info *miclist)
{
	...
	if (!is_mic_knc(miclist) && mtu != NULL) {
		display(PERROR, "MTU option is available only on KNC\n");
		EXIT_ERROR(EINVAL);
	}
...
void fstab_add(char *base, char *edir, char *mdir, char *server);
...
mod_nfshost(struct mic_info *mic)
{
	...
	if (mic->config.rootdev.type == ROOT_TYPE_SPLITNFS) {
		...
		if ((usrname = strchr(mic->config.rootdev.nfsusr, ':') + 1) != NULL) {
			...
			fstab_add(mic->config.filesrc.mic.dir, usrname, "/usr", hostIP);
			fstab_add(nfsname, usrname, "/usr", hostIP);
		}
	} else {
		display(PFS, "%s: Update RootDevice in %s\n", mic->name, mic->config.name);
...
void
gen_compat_ipinfo(struct mic_info *mic)
{
	char filename[PATH_MAX];
	...
	snprintf(dirname, PATH_MAX, "/opt/intel/mic/filesystem/%s/etc/sysconfig/network", mic->name);
	mpssut_filename(&mpssenv, NULL, filename, PATH_MAX,
				"/opt/intel/mic/filesystem/%s/etc/sysconfig/network/ifcfg-mic0", mic->name);

	if (mic->config.net.type == NETWORK_TYPE_BRIDGE) {
		display(PWARN, "%s: Network uses DHCP - IDB will be useless - "
				"not createing compatibility network file %s\n", mic->name, filename);
		unlink(filename);
		return;
	}

	display(PWARN, "%s: Generating compatibility network config file %s for IDB.\n",
			mic->name, filename);
	display(PWARN, "      This may be problematic at best and will be removed in a future "
			"release, Check with the IDB release.\n");

	if (stat(filename, &sbuf) == 0)
		unlink(filename);
	else
		if ((err = mpssut_mksubtree(&mpssenv, "", dirname, 0, 0, 0755)) && (err != EEXIST)) {
			display(PERROR, "Failed to create directory %s for IDB compatibility network "
				"configuration file\n", dirname);
		}
	...
	case NETWORK_TYPE_STATPAIR:
		...
		if (mic->config.net.prefix != NULL)
			fprintf(fp, "PREFIX=%s\n", mic->config.net.prefix);
		else
			fprintf(fp, "PREFIX=24\n");

		if (is_mic_knc(mic) && mic->config.net.mtu != NULL)
			fprintf(fp, "MTU=%s\n", mic->config.net.mtu);
		break;

	case NETWORK_TYPE_BRIDGE:
		// ignore this condition because the reason for the comat ifcfg file does not
		// work in this mode anyways.
		break;
	}
...
gen_ipinfo(struct mic_info *mic, int update)
{
	...
	char *header;
	char *eth_if;
	gen_compat_ipinfo(mic);	// HACK HACK HACK HACK for IDB to find IP addresses
