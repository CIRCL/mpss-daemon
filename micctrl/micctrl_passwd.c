...
int change_pw(char *basedir, int ruid, char *name, char *new, char *oldpassword, char *passwd_str);
int passwd_policy_check(char *passwd);
void display(unsigned char level, char *format, ...);

int parse_pwfile(char *user, char **pw, int *uid, int *gid, char **name, char **home, char **app);
...
int
change_pw(char *base, int ruid, char *name, char *new, char *oldpassword, char *passwd_str)
{
	...
	*/
	...
	while ((ret = fgets(line, PWBUF_SIZE, ifp))) {
		...
		if (!strcmp(user, name)) {
			...
			fprintf(ofp, "%s:%s:%s:%s:%s:%s:%s:%s:\n", user, new, lastd, minage, maxage,
				     passwarn, inactive, expire);
			if (passwd_str != NULL)
				sprintf(passwd_str, "%s:%s:%s:%s:%s:%s:%s:%s:\n", user, new, lastd, minage, maxage,
						    passwarn, inactive, expire);
		} else {
