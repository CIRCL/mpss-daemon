...
void set_pfsroot(struct mic_info *mic, int type, char *target);
 ...
 */
char *root_types[] = {"ramfs", "staticramfs", "nfs", "splitnfs", "pfs"};
...
rootdev(struct mic_info *miclist, int miccnt, char *type, char *target, char *usr, char *server, int options)
{
	...
	while (mic != NULL) {
		...
		} else if (!strcasecmp(type, "pfs")) {
			set_pfsroot(mic, ROOT_TYPE_PFS, target);
		} else {
...
int nfs_opt_check(char *nfs_opt)
{
	...
	if ((subopts = malloc(strlen(nfs_opt) + 1)) == NULL) {
		memory_alloc_error();
	}
...
add_nfsmount(struct mic_info *mic, char *target, char *server, char *mountdir, char *nfs_opt)
{
	...
	strncpy(buffer, target, PATH_MAX);
...
void
set_pfsroot(struct mic_info *mic, int type, char *target)
{
        char targetname[PATH_MAX];
	char pfstarget[PATH_MAX];
	char mictarget[PATH_MAX];
	char command[PATH_MAX];
	char mpssdconfig[PATH_MAX];
	int err;
	FILE *ifp;
	struct stat sbuf;
	...
	//get the location of persistent file system
        if (target == NULL) {
                snprintf(targetname, PATH_MAX, "%s/%s", mpssenv.srcdir, DEFAULT_PFS_KNL);
        } else {
		snprintf(targetname, PATH_MAX, "%s", target);
        }
	snprintf(mictarget, PATH_MAX, "%s/%s/%s", mpssenv.vardir, mic->name, DEFAULT_PFS_KNL);

	//copy file from /usr/share/mpss/boot to /var/mpss/micX
	stat(targetname, &sbuf);
	if (mpssut_copyfile(mictarget, targetname, &sbuf)) {
		display(PERROR, "%s: Failed to copy from %s to %s\n", mic->name, targetname, mictarget);
		return;
	}

	//extract gzip file
	snprintf(command, PATH_MAX, "/bin/gzip -d %s", mictarget);
	system(command);

	//remove from filename extension .gz
	mictarget[strlen(mictarget) - 3] = '\0';

	//update config file with the name of persistent file system
	mpssut_realloc_fill(&mic->config.rootdev.target, mictarget);
	if ((err = mpss_update_config(&mpssenv, mic->config.name, NULL, descriptor, "%s %s %s\n\n",
		"RootDevice", fstypes[type], mic->config.rootdev.target))) {
		display(PERROR, "%s: Cannot update config file\n", mic->name);
		return;
	}

	//update config file /etc/mpss/mpssd000.conf
	snprintf(mpssdconfig, PATH_MAX, "%s/mpssd%03d.conf", DEFAULT_CONFDIR, mic->id);
	if ((ifp = fopen(mpssdconfig, "w")) == NULL) {
		display(PERROR, "%s: Cannot open file %s\n", mic->name, mpssdconfig);
		return;
	}
	snprintf(pfstarget, PATH_MAX, "VIRTBLK_BACKEND=%s", mic->config.rootdev.target);

	fwrite(pfstarget, strlen(pfstarget), 1, ifp);
	fclose(ifp);
}
